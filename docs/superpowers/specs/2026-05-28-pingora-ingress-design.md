...
**Port 80 (`web`) `request_filter`:**
1. If path starts with `/.well-known/acme-challenge/` → look up token in
   `ArcSwap<ChallengeMap>`; respond `200` with the key authorization (or `404`).
...
- **Listener:** `:443` built via `TlsSettings` plus a `TlsAccept` callback. The
  callback reads `ArcSwap<CertStore>` (SNI → parsed cert/key) at handshake.
  Renewal swaps the `ArcSwap` with zero restart and zero dropped connections.
- **Issuance (`acme.rs`):** on a `tls_enabled` service create/update or domain
  verification (ADR-013), enqueue an ACME order against the configured directory
  (Let's Encrypt prod by default; staging via env for tests). HTTP-01:
  - Publish `token → key_authorization` into `ArcSwap<ChallengeMap>`.
  - Port-80 `request_filter` serves it (see flow above).
  - On `valid`, fetch the cert chain, persist cert + key to
    `<data_dir>/tls/<domain>/{fullchain.pem,key.pem}` at mode `0600`, and swap into
    `CertStore`.
...
- Remove `bridge_allocator`, `bridge_manager`, `bridge_supervisor`,
  `ingress_options` (render flavor). Add `ingress: Arc<IngressState>`.
- `SharedRoutes` (the `Arc<Mutex<BTreeMap<String, RouteSpec>>>` API snapshot)
  stays for `GET /v1/ingress/routes`; `IngressState` is the live routing source.
  Deploy/stop/scale paths update both (or `IngressState` derives the snapshot).
- The autoscaler (ADR-018) rebinds its activator hook and idle reaper to
  `IngressState` — same `ActivationHook` trait, new owner. `next_socket`,
  `healthy_count`, `last_activity`, `set_replica_healthy`, `add_replica`,
  `remove_replica` keep their signatures so deploy/coordinator call sites change
  minimally.

### Deploy / routes integration
...
- **Remove env:** `DENIA_TRAEFIK_IMAGE`, `DENIA_TRAEFIK_DYNAMIC_CONFIG`, any
  Traefik static-config paths.
...
- **Cargo.toml:** add `pingora` + `pingora-proxy` (pinned), `instant-acme`. Pick
  ...
  default and supports the `TlsAccept` cert callback; confirm during impl. Remove
  any Traefik-OCI-only deps if now unused (verify the OCI puller is still needed
  for workload images — it is; keep it).
...
- Amend ADR-007: ingress no longer uses a file provider / dynamic YAML; routing
  is in-memory; `GET /v1/ingress/config` removed; the in-memory snapshot remains
  canonical for `/v1/ingress/routes`.
...
1. UDS-upstream support in the pinned Pingora version (decides bridge full-delete
   vs thin shim).
2. Exact `TlsAccept` / dynamic-cert callback API in the pinned version.
3. Whether Pingora `Server` can be built without installing OS signal handlers.
