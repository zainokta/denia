...
| A6 | MINOR | `build_server`/`Server::new(None).expect()` panics; prefer `Result` per project guidance "no panics for expected failures". | ✅ fixed (Chunk C, `server.rs:120` returns `Result<Server, ServerBuildError>`; `main` handles bind failure non-fatally) |
...
| C5 | MINOR | `DENIA_ACME_DIRECTORY_URL` defaults to LE **production** — document the staging override to avoid rate-limit burns in non-prod. | ✅ documented (ADR-020 + README + project guidance) |
...
- **Docs:** ADR-020 written; ADR-016 marked Superseded (header + index); root `project guidance`/`project guidance` + `README.md` ingress sections rewritten for Pingora.
